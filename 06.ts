...
const points: string[] = Array(26).fill(97).map((n, i) => String.fromCharCode(n + i));
...
function answer(input) {
  ...
  for (const pair of coords) {
    if (!minX || pair[0] < minX) minX = pair[0];
    if (!minY || pair[1] < minY) minY = pair[1];
    if (!maxX || pair[0] > maxX) maxX = pair[0];
    if (!maxY || pair[1] > maxY) maxY = pair[1];
  }
  ...
  const totals: {[coord: string]: number} = pointMap.flat().reduce((counts, point) => {
...
const answer2 = (input: string, maxDist: number) => {
  const coords = parseInput(input);
...
const stringCoord = ([x, y]: number[]) => `(${x},${y})`;
