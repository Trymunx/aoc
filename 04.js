...
const puzzleInput = `[1518-04-22 00:52] wakes up
...
function firstAnswer(input) {
  ...
  let dates = rows.map(row => {
    return {
      date: row.replace(/^\[(\d{4})-(\d{2})-(\d{2})\s+(\d{2}):(\d{2})\].*/, "$1$2$3$4$5"),
      text: row.replace(/^\[.*\]\s(.*)$/, "$1"),
    }
  });
  ...
  let guard;
  let max = 0;
  let times = dates.reduce((obj, log) => {
    if (/Guard #/.test(log.text)) {
      guard = log.text.replace(/^.*#(\d+).*/, "$1");
      if (!obj[guard]) {
        obj[guard] = {
          timeAsleep: 0,
          minutesAsleep: {},
          snooziest: {mins: 0, ts: 0},
        };
      }
      return obj;
    }
    if (/falls asleep/.test(log.text)) {
      obj[guard].asleepAt = log.date;
    } else if (/wakes up/.test(log.text)) {
      let timeAsleep = log.date - obj[guard].asleepAt;
      obj[guard].timeAsleep += timeAsleep;

      let asleepMin = parseInt(
        obj[guard].asleepAt.replace(/.*(\d{4})$/, "$1")
        .replace(/^0*([^0]+.*)/, "$1")
      );
      

      // console.log("asleep:", obj[guard].asleepAt, asleepMin);
      let awakeMin = parseInt(
        log.date.replace(/.*(\d{4})$/, "$1")
        .replace(/^0*([^0]+.*)/, "$1")
      );
      
      // console.log("woke:", log.date, awakeMin);
      for (let i = asleepMin; i < awakeMin; i++) {
        // console.log(i);
        if (obj[guard].minutesAsleep[i]) {
          obj[guard].minutesAsleep[i]++;
          if (obj[guard].minutesAsleep[i] > obj[guard].snooziest.mins) {
            obj[guard].snooziest.mins = obj[guard].minutesAsleep[i];
            obj[guard].snooziest.ts = i;
          }
        } else {
          obj[guard].minutesAsleep[i] = 1;
        }
      }
      // console.log(obj[guard]);


      max = Math.max(obj[guard].timeAsleep, max);
    }
    return obj;
  }, {});
  // console.log(times);
  ...
  Object.keys(times).forEach(key => {
    if (times[key].timeAsleep === max) {
      winner = key * times[key].snooziest.ts;
    }
  });
  return winner;
}
...
function secondAnswer(input) {
      ...
      for (let i = asleepMin; i < awakeMin; i++) {
        ...
        if (obj[guard].minutesAsleep[i]) {
          ...
          if (obj[guard].minutesAsleep[i] > obj[guard].snooziest.mins) {
            obj[guard].snooziest.mins = obj[guard].minutesAsleep[i];
            obj[guard].snooziest.ts = i;
            max = Math.max(obj[guard].minutesAsleep[i], max);
          }
        } else {
          obj[guard].minutesAsleep[i] = 1;
        }
  ...
  let winner;
  Object.keys(times).forEach(key => {
    if (times[key].snooziest.mins === max) {
      winner = key * times[key].snooziest.ts;
    }
  });
  return winner;
}
// firstAnswer(example1);
console.log(firstAnswer(puzzleInput));
// secondAnswer(example1);
