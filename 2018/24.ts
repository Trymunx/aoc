export {}; // This is to make ts realise that this file is a module, so things are not global
...
type Affinity = "Infection" | "Immune System";
...
type GenericGroup = Omit<Group, 'affinity'>;

type TargetPair = [number, number | undefined];

const addAffinity = (affinity: Affinity, groups: GenericGroup[]): Group[] =>
  groups.map(group => ({...group, affinity}));

const parseInput = (input: string): { immuneSysGroups: Group[], infectionGroups: Group[] } => {
  const armies = input.split("\n\n");
  if (armies.length !== 2) throw new Error(`Got ${armies.length} armies.`);
...
const parseGroup = (s: string): GenericGroup => {
...
const orderGroupsForTargeting = (...groups: Group[]): Group[] =>
  groups.sort((a, b) => {
    const diff = b.effectivePower - a.effectivePower;
    if (diff === 0) return b.initiative - a.initiative;
    else return diff;
  });

const orderTargetPairsForAttacking = (tps: TargetPair[]): TargetPair[] =>
  tps.sort((a, b) => b[0] - a[0]);

const chooseTargets = ({ targetingOrder, immuneSysGroups, infectionGroups }: {
  targetingOrder: Group[];
  immuneSysGroups: Group[];
...
const selectTarget = (attacker: Group, defenders: Group[]): { targetPair: TargetPair, remainingDefenders: Group[]} => {
  let bestTarget: {
    target: Group;
    damage: number;
  } = {
  ...
  const targetPair: TargetPair = [attacker.initiative, bestTarget.target.initiative]
  const remainingDefenders = defenders.filter(d => d !== bestTarget.target);
  return { targetPair, remainingDefenders };
}

const groupFromNum = (allGroups: Group[], initiative: number): Group | undefined => allGroups.find(
  g => g.initiative === initiative
);

const performAttacks = (targetPairs: TargetPair[], allGroups: Group[]) => {
...
const calculateDamage = (attacker: Group, defender: Group): number => {
...
function main(input: string) {
  ...
  let allGroups: Group[] = [...immuneSysGroups, ...infectionGroups];
