...
function part1(input: string): number {
  ...
  // Make grid of points, with the counts of how many wires are there
  const grid: { [coord: string]: boolean; } = {
    "0,0": true,
  };
  let lastPoint = [0, 0];

  for (const point of wire1) {
    const dir = point[0];
    let dist: number = parseInt(point.slice(1), 10);
    ...
    while (dist-- > 0) {
      ...
      grid[`${newPoint[0]},${newPoint[1]}`] = true;
      lastPoint = newPoint;
    }
  }

  const crossedPoints: Array<[number, number]> = [];
  lastPoint = [0, 0];

  for (const point of wire2) {
    const dir = point[0];
    let dist: number = parseInt(point.slice(1), 10);
    ...
    while (dist-- > 0) {
      ...
      if (grid[`${newPoint[0]},${newPoint[1]}`]) {
        crossedPoints.push(newPoint);
      }
      lastPoint = newPoint;
    }
  }
  return Math.min(...crossedPoints.map(point => Math.abs(point[0]) + Math.abs(point[1])));
}
...
function part2(input: string): number {
  ...
  // Make grid of points, with the counts of how many wires are there
  const grid: { [coord: string]: number; } = {
    "0,0": 0,
  };
  let lastPoint = [0, 0];
  let stepCounter = 0;
  ...
  for (const point of wire1) {
    ...
    while (dist-- > 0) {
      ...
      grid[`${newPoint[0]},${newPoint[1]}`] = ++stepCounter;
      lastPoint = newPoint;
    }
  }

  const intersectionTotals: number[] = [];
  lastPoint = [0, 0];
  stepCounter = 0;
  ...
  for (const point of wire2) {
    ...
    while (dist-- > 0) {
      ...
      if (grid[`${newPoint[0]},${newPoint[1]}`]) {
        intersectionTotals.push(grid[`${newPoint[0]},${newPoint[1]}`] + stepCounter);
      }

      lastPoint = newPoint;
    }
  }
  return Math.min(...intersectionTotals);
}
